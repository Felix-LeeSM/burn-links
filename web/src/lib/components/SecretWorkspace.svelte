<script lang="ts">
	...
	import { createSecretAPIClient, createShareURL, type TTLSeconds } from '$lib/api/secrets';
	import { decryptText, encryptText, type EncryptedTextPayload } from '$lib/crypto/text';

	let plaintext = $state('');
	let passphrase = $state('');
	let openSecretID = $state('');
	let openPassphrase = $state('');
	let decryptedText = $state('');
	let shareURL = $state('');
	let status = $state('');
	let ttlSeconds = $state<TTLSeconds>(3600);
	...
	onMount(() => {
		...
		if (match) {
			openSecretID = decodeURIComponent(match[1]);
		}
	});
	...
	async function createSecret() {
		...
		try {
			...
			const created = await api.createTextSecret(encrypted, ttlSeconds);
	...
	async function openSecret() {
		...
		try {
			const payload = await api.getSecret(openSecretID);
			decryptedText = await decryptText(payload as EncryptedTextPayload, openPassphrase);
			await api.consumeSecret(openSecretID);
			status = 'Opened';
		} catch (error) {
			status = messageFrom(error);
		}
...
<main class="shell">
	...
	<section class="workspace" aria-label="Secret workspace">
		<form class="panel" onsubmit={(event) => event.preventDefault()}>
			...
			<button type="button" onclick={createSecret}>Create Link</button>
		</form>
		...
		<form class="panel" onsubmit={(event) => event.preventDefault()}>
			...
			<label for="open-secret-id">Secret ID</label>
			<input id="open-secret-id" bind:value={openSecretID} autocomplete="off" />
			...
			<input
				...
				bind:value={openPassphrase}
			/>

			<button type="button" onclick={openSecret}>Open Once</button>
		</form>
	</section>

...
for _ in $(seq 1 30); do
  if curl -fsS http://127.0.0.1:8222/varz >/tmp/flick-nats-varz.json; then
    break
  fi
  sleep 1
done
...
if command -v node >/dev/null 2>&1 && node -e "process.exit(0)" >/dev/null 2>&1; then
  node -e "const v=JSON.parse(require('fs').readFileSync('/tmp/flick-nats-varz.json','utf8')); if (!v.jetstream) process.exit(1)"
else
  grep -q '"jetstream"' /tmp/flick-nats-varz.json
fi
